        ...
        {particleConfigs.map((particle, i) => (
          <div
            ...
            style={{
              ...
              transform: isMounted
                ? `translate(${Math.sin(time + i) * 20}px, ${Math.cos(time + i) * 20}px)`
                : 'translate(0px, 0px)',
              opacity: 0.6
            }}
          />
      ...
      <div className="relative z-20 py-4 px-4">
        ...
        <div className="text-center mb-6">
          ...
          <div className="flex justify-center items-center gap-2 mt-2">
            <div className="w-8 h-0.5" style={{ background: `linear-gradient(to right, transparent, var(--octwave-via))` }} />
            <div className="text-purple-400 dark:text-purple-300 text-xs font-mono tracking-wider font-semibold">Octwave 2.0</div>
            <div className="w-8 h-0.5" style={{ background: `linear-gradient(to left, transparent, var(--octwave-via))` }} />
          </div>
        </div>

        {/* Main Timeline Container */}
        <div className="max-w-7xl mx-auto relative">
          {/* Central Neural Network - hidden on mobile */}
          <div className="hidden sm:block absolute left-1/2 top-0 bottom-0 transform -translate-x-1/2 w-4">
          ...
          <div className="space-y-5 sm:space-y-8">
            {timelineEvents.map((event, index) => (
              ...
              >
                {/* Wrapper to center + shrink on mobile */}
                <div className="w-full sm:w-5/12 flex justify-center sm:justify-start">
                  <div className="max-w-[300px] sm:max-w-none w-full scale-[0.9] sm:scale-100 origin-center">
                    {/* Animated container for entrance effects */}
                    <div
                      className={`transform transition-all duration-1000 ${
                        visibleItems.includes(event.id)
                          ? 'translate-x-0 opacity-100 scale-100'
                          : 'opacity-0 scale-95 ' + (index % 2 === 0 ? 'sm:-translate-x-16' : 'sm:translate-x-16')
                      }`}
                    ...
                    >
                      <div className="relative">
                        ...
                        >
                          <defs>
                            <linearGradient id={`hologram-m-${event.id}`} x1="0%" y1="0%" x2="100%" y2="100%">
                            ...
                            <pattern id={`scanlines-m-${event.id}`} patternUnits="userSpaceOnUse" width="4" height="4">
                          ...
                          <path
                            ...
                            fill={`url(#hologram-m-${event.id})`}
                            stroke={event.color}
                            strokeWidth="2"
                            filter={`url(#hologramGlow-m-${event.id})`}
                          />
                          ...
                          <g transform={`translate(${index % 2 === 0 ? '30' : '340'}, 35)`}>
                            <circle
                              ...
                              filter={`url(#hologramGlow-m-${event.id})`}
                            />
                            <circle cx="12" cy="12" r="10" fill={event.color} fillOpacity="0.1" className="animate-pulse" />
                          ...
                          {/* Content */}
                          <foreignObject x={index % 2 === 0 ? '70' : '100'} y="30" width="260" height="170">
                            <div className="text-gray-800 dark:text-gray-200 p-2">
                              <div className="flex items-center gap-1 mb-1">
                                <div className="w-1 h-1 rounded-full animate-pulse" style={{ backgroundColor: event.color }} />
                                <span className="text-xs font-mono text-gray-500 dark:text-gray-400 tracking-wide font-semibold">
                                  #{event.id}
                                </span>
                              </div>
                        ...
                        {/* DESKTOP SVG */}
                        <svg
                          className={`hidden sm:block w-full h-40 ${index % 2 === 0 ? 'sm:ml-0' : 'sm:mr-12'}`}
                          viewBox="0 0 400 130"
                          preserveAspectRatio="xMidYMid meet"
                          style={{ overflow: 'visible' }}
                        >
                          <defs>
                            <linearGradient id={`hologram-d-${event.id}`} x1="0%" y1="0%" x2="100%" y2="100%">
                              <stop offset="0%" stopColor={event.color} stopOpacity="0.05" />
                              <stop offset="50%" stopColor={event.color} stopOpacity="0.15" />
                              <stop offset="100%" stopColor={event.color} stopOpacity="0.05" />
                            </linearGradient>
                            <filter
                              id={`hologramGlow-d-${event.id}`}
                              x="-20%"
                              y="-20%"
                              width="140%"
                              height="140%"
                            ...
                            <pattern id={`scanlines-d-${event.id}`} patternUnits="userSpaceOnUse" width="4" height="4">
                              <rect width="4" height="2" fill={event.color} opacity="0.05" />
                              <rect y="2" width="4" height="2" fill="transparent" />
                            </pattern>
                          </defs>
                          ...
                          <path
                            ...
                            fill={`url(#hologram-d-${event.id})`}
                            stroke={event.color}
                            strokeWidth="2"
                            filter={`url(#hologramGlow-d-${event.id})`}
                          />
                          ...
                          {/* Content */}
                          <foreignObject x={index % 2 === 0 ? '70' : '100'} y="30" width="230" height="170">
                            <div className="text-gray-800 dark:text-gray-200 p-2">
                              <div className="flex items-center gap-1 mb-1">
                                <div className="w-1 h-1 rounded-full animate-pulse" style={{ backgroundColor: event.color }} />
                                <span className="text-xs font-mono text-gray-500 dark:text-gray-400 tracking-wide font-semibold">
                ...
                {/* Central Neural Node - hidden on mobile */}
                <div className="hidden sm:block absolute left-1/2 transform -translate-x-1/2 z-30">
                  <div
                  ...
                  >
                    <svg width="60" height="60" viewBox="0 0 60 60" style={{ overflow: 'visible' }}>
                      ...
                      {/* Year */}
                      <text x="30" y="26" textAnchor="middle" className="text-xs font-mono font-bold" fill={event.color}>
                        {event.year}
                      </text>

                      {/* ID */}
                      <text x="30" y="36" textAnchor="middle" className="text-sm font-bold fill-gray-800 dark:fill-gray-200">
                        {event.id}
                      </text>
                    </svg>
                  </div>
                ...
                {/* Neural Connection - hidden on mobile */}
                <div className="hidden sm:block absolute left-1/2 top-1/2 transform -translate-y-1/2">
                  <div className={`${index % 2 === 0 ? '-translate-x-full' : 'translate-x-0'}`}>
                    <svg width="120" height="6" viewBox="0 0 120 6" style={{ overflow: 'visible' }}>
                      <defs>
